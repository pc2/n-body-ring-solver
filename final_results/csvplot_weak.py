...
for x_col,y_col, ifrom, ito in zip(col_x_axis, col_y_axis, i_from, i_to):
    ...
    if i == 0:
        ...
        markers='C3--^'
        fill_style='full'
        marker_face_color='C0'
    elif i == 5:
        markers='C3--s'
        fill_style='full'
        marker_face_color='C1'

    if i < 4:
        if i != 3:
            ax1.plot(x_axis_list, y_axis_list,markers,fillstyle=fill_style,linewidth=line_width,markevery=1,label=legend[i])
        else:
            line1, = ax1.plot(x_axis_list, y_axis_list,markers,fillstyle=fill_style,linewidth=line_width,markevery=1,label=legend[i])
            line1.set_dashes([4,2,4,2,0,3])
    else:
        ax2.plot(x_axis_list, y_axis_list,markers,fillstyle=fill_style,linewidth=line_width,markevery=1,markerfacecolor=marker_face_color,label=legend[i])
    i += 1

#ax1.set_xscale("log",basex=2)

ax2.set_yscale('log', basey=10)
ax2.set_ylim(0.05,0.4*10**4)
#ax2.set_ylim(-10,330)
...
if legend:
    ...
    ax1.legend(lines,labels, loc=0, fontsize='small')
