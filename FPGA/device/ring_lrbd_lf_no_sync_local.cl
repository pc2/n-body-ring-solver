...
//Use compact double4 instead: xyz are the position, w the coefficient, if w == 0.0 the particle is invalid
...
// alternative implementation could provide 1x1 PEs for full solver
    ...
    channel particle_t io_chan_input_part    __attribute__((io("kernel_input_ch0"), depth(PRELOAD_REMOTE_SIZE)));
    channel particle_t io_chan_output_part   __attribute__((io("kernel_output_ch1"), depth(PRELOAD_REMOTE_SIZE)));
    channel particle_t io_chan_input_force   __attribute__((io("kernel_input_ch2"), depth(REMOTE_BLOCK_SIZE)));
    channel particle_t io_chan_output_force  __attribute__((io("kernel_output_ch3"), depth(REMOTE_BLOCK_SIZE)));
#endif
        ...
        for(uint dev = 0;dev < comm_sz;dev++)
        {
            ...
            for(uint r_block=0; r_block<r_block_max; r_block++)
            {
                ...
                for(uint l_block=0; l_block<l_block_max; l_block++)
                {
                    ...
                    double3             b_local_forces   [LATENCY_FACTOR][PE_LOCAL_CNT_ALLOC][PE_LOCAL_SIZE] = {nan((ulong)0)};
                    double3             b_remote_forces   [LATENCY_FACTOR][PE_REMOTE_CNT_ALLOC][PE_REMOTE_SIZE] = {nan((ulong)0)};
                    #else
                                     ...
                                     )
{
    ...
    for(uint s=0; s<steps*(lf_steps-1); s++)
    {
        ...
        for(uint r=0; r<total_trips; r++)
        {
            ...
            {
                in_part = read_channel_intel(io_chan_input_part);
                #ifdef COMPILE_EMULATION
            ...
            {
                write_channel_intel(io_chan_output_part, out_part);
                #ifdef COMPILE_EMULATION
            ...
            for(uint r_block=0; r_block<r_block_max; r_block++)
            {
                ...
                for(uint l_block = 0;l_block < l_block_max;l_block++)
                {
                    for(uint rr=0; rr<REMOTE_BLOCK_SIZE; ++rr)
                    {
                        ...
                        double3 remote_force = read_channel_intel(chan_compute_to_remote_force_cache);
                        #ifdef COMPILE_EMULATION
                        ...
                        double3 old_force = dev == 0 && l_block == 0 ? ZERO : (l_block == 0 ? read_channel_intel(chan_remote_force_comm_to_remote_force_cache) : force_buffer[rr]);
                        #ifdef COMPILE_EMULATION
                        if(!(dev == 0 && l_block == 0))
                        {
                            if(l_block == 0)
                        ...
                        if(l_block == l_block_max - 1)
                        {
                            ...
                            write_channel_intel(io_chan_output_force, (double4)(force_buffer[rr],0.0));
                            #ifdef COMPILE_EMULATION
                            r_io_channel_out_count++;
                            #endif
                        }
                         ...
                         )
{
    ...
    #ifdef COMPILE_EMULATION
    double c[MAX_LF_COEFFICIENTS] = {nan((ulong)0)};
    double d[MAX_LF_COEFFICIENTS] = {nan((ulong)0)};
    particle_t local_part[MAX_N] = {nan((ulong)0)};
    double3 local_vel[MAX_N] = {nan((ulong)0)};
    double local_mass[MAX_N] = {nan((ulong)0)};
    #else
